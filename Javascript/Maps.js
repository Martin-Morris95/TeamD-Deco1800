var map;
//root node of tree for markers
var root;
//current node of tree for markers
var current;
//Manages showing and hiding territories
var territoryManager;
//Manages showing and hiding deployment
...
$(document).ready(function() {
  ...
  $("#backButton").click(function() {
    //Go up a level in the marker tree
    current.hideChildren();
    current.hideChildrenPopups();
    current.hidePopup();
    current = current.getParent();
    current.showChildren();
    //If the current node is the root node then the back button is no longer needed
    if(current.isRoot()) {
  ...
  //Handles a year being selected on the timeline
  $("#timeline p").click(function() {
    //update visual for current year in timeline
    $(".current").removeClass("current");
    $(this).addClass("current");
    var year = parseInt($(this).text());
    //update year for markers, territories and deployment
...
//Start of marker classes ------------------------------------------------------------------------------

/*
  Markers on the map have a tree structure
  This class is a single node in that tree

  Parameters:
    zoom (number): zoom level when this node is the current node
*/
function Node(zoom) {
  this.zoom = zoom;
  this.children = [];
...
/*
  ...
  Parameters:
    marker (Marker): child to add to this node
*/
...
/*
  Controls whether this node can show its children

  Parameters:
    active (bool): true if this node should be active, false otherwise
*/
...
/*
  Hides all of this node's children on the map
*/
...
/*
  Shows all of this node's children on the map
*/
...
/*
  Hides any popups that children of this node have displayed
*/
...
/*
  Changes the zoom level of the map to the zoom level of this node

  Returns:
    bool: indicates whether the zoom of the map was changed
*/
...
/*
  Returns whether this node is at the bottom of the tree
  If the node has no children then it is at the bottom
*/
...
/*
  Updates the current year of this node

  Parameters:
    year (number): the currently selected year
*/
...
/*
  Node in the tree that has a marker
  Inherits from Node

  Parameters:
    latt (number): latitude of marker
    long (number): longitude of marker
    year (array): years that this marker can be displayed, empty list if marker can be displayed in any year
    zoom (number): zoom level when this node is the current node, null if zoom level should not be changed
...
/*
  ...
  Hides the nodes at the current level and shows its children if it has any
  If this marker has a popup it is shown
*/
Marker.prototype.click = function(event) {
  if(this.changeZoom()) {
    //only go down a level of the tree if the zoom level is changed
    //keep track of current node
    current = this;
    //centre on marker position
    updateMap(this.marker.getPosition().lat(), this.marker.getPosition().lng());
    //at the second level of markers, the map zoom can be controlled with the scroll wheel
  ...
  this.showPopup();
  if(!this.isBottom()) {
    //hide markers on the same level as this marker
    if(this.parent != null) {
...
/*
  Sets the parent of this marker
*/
...
/*
  Gets the parent of this marker
*/
...
Marker.prototype.show = function() {
  //only display the marker if the current year is correct or if the marker does not have a specific year
...
/*
  Gets the position of the marker on the map
*/
...
/*
  Shows the popup for this marker if it has one
*/
...
/*
  Hides the popup for this marker
*/
...
/*
  Adds a year that this marker can be displayed
*/
Marker.prototype.addYear = function(year) {
  this.year.push(year);
}

//End of marker classes --------------------------------------------------------------------------------

//Start of popups --------------------------------------------------------------------------------------

/*
  Popup that is displayed when a marker is clicked

  Parameters:
    hidden (bool): true if popup should start hidden, false otherwise
*/
...
/*
  Helper function for popup class
  Creates the DOM element for displaying the popup

  Parameters:
    title (string): title of popup
    text (strign): main text of popup
    statistics (dictionary): statistics to display
...
/*
  Question that is displayed at bottom of screen
*/
...
/*
  Change the question that is displayed

  Parameters:
    question (string): the new question
    answer (string): answer to the new question
*/
...
/*
  Helper function for question class
  Creates the DOM element for displaying the question

  Parameters:
    question (string): question to display
    callback (function): function to call when user enters answer
*/
...
//Start of class for managing deployment and territories -----------------------------------------------
...
//End of class for managing deployment and territories -----------------------------------------------
 
//Start of territory class ---------------------------------------------------------------------------
...
//End of deployment class ----------------------------------------------------------------------------

//Start of menu for filtering data ---------------------------------------------------------------------
...
$('input[type=checkbox]').change(function(){
    if(this.checked){
        ...
        if(this.id == "battles" ){
            //turn battle data off
            current.setActive(false);
            current.hideChildren();
        }else if(this.id == "deployment"){
            //turn deployments off
            deploymentManager.setActive(false);
        }else if(this.id == "enemy"){
            //turn territories off
...
//End of menu for filtering data ----------------------------------------------------------------------
...
function initMap() {
  ...
  var questions = [
   ...
   // ["What countries were involved in Gallipoli", "Australia, Britain, New Zealand, Turkey"],
    ...
    ["How Many casualties were there in the battle of Lone Pine, Turkey","2000"],
    ["What date did the battle of Lone Pine, Turkey (1915) take place","6th of August"],
    ["What time did the Australians use artillery bombardment in Israel against Turkish positions","4:30pm"],
    ["How Many casualties were there in the battle of Nek, Turkey, 1915","234"],
    ["What animals were used in the battle of Nek, Turkey 1915","Horses"],
    ["What was the Australian brigade called in the battle of Nek, Turkey 1915","3rd Light Horse Brigade"],
  ...
  //get list of questions already answered by user and remove them from the questions
  ...
  /*
    gets a random question from the list of questions, returns null if there are no more questions
  */
...
/*
  Moves the position of the map

  Parameters:
    latt (number): latitude to move map to
    long (number): longitude to move map to
*/
